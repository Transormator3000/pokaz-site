...
(function() {
    ...
    const scriptName = scriptSrc.split('/').pop().replace('.js', '') || 'slider-met';
    ...
    window.SLIDER_MET_SCRIPT_NAME = scriptName;
    
    console.log(`🏷️ МОДУЛЬ 1: ${scriptName} зарегистрирован в WAIT шлюзе`);
})();

// ===== МОДУЛЬ 2: READY ОТПРАВЩИК =====
const SliderMetReadyReporter = {
    scriptName: window.SLIDER_MET_SCRIPT_NAME,
    isReported: false,
...
const SliderMetPreloader = {
    scriptName: window.SLIDER_MET_SCRIPT_NAME,
    
    init() {
        console.log(`🔄 МОДУЛЬ 3: ${this.scriptName} preloader запущен`);
        
        // Инициализируем все компоненты металлоконструкций
        this.initializeSliderMetComponents(() => {
            // По завершении сигналим модулю 2
            this.signalComplete();
        });
    },
    
    initializeSliderMetComponents(callback) {
        try {
            console.log('🔄 Начинаем инициализацию модулей металлоконструкций...');
            
            // Инициализируем все модули металлоконструкций
            const modules = {
            ...
            console.error('❌ Ошибка при инициализации модулей металлоконструкций:', error);
...
document.addEventListener('DOMContentLoaded', function() {
    console.log('🚀 Slider-Met: Запускаю новую архитектуру');

    // Инициализируем модули в правильном порядке
    SliderMetReadyReporter.init();
    SliderMetPreloader.init();
});

// ===== ОТЛАДОЧНЫЕ ФУНКЦИИ =====
function getSliderMetStatus() {
    console.log('📊 СТАТУС Slider-Met новой архитектуры:');
    console.log('  ReadyReporter:', SliderMetReadyReporter.isReported ? 'ГОТОВ' : 'ЖДЕТ');
    console.log('  LSAMBase:', window.LSAMBase ? window.LSAMBase.getStats() : 'НЕ ГОТОВ');
}

console.log('✅ Slider-Met: Новая архитектура запущена');
